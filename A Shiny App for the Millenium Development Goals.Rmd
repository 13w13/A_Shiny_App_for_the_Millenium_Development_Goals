...
goalD[unique(goalD$Series_Name),.N]
goalD[unique(goalD$Series_Code),.N]
...
unique(metrics[,2])
```
We should merge them into the first column
```{r}
mapply(metrics$V1,paste(metrics$V2) %>% head
...
# Remove parenthesis
metrics <- substring(metrics, 2, nchar(metrics)-1)
#Verify results
k[10000,]
goalD[10000]
```
...
Count how many maches tables with Series Code and how many none

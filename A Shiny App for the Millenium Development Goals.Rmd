...
mean_region_goalD %>% head
```

### 2.4.2 For mean_income_goalD
```{r}
mean_income_goalD=goalD[, lapply(.SD, mean(), na.rm=TRUE), by=list(Income_group),.SDcols=c("1972","1973","1974","1975","1976","1977","1978","1979","1980","1981","1982","1983","1984","1985","1986","1987","1988","1989","1990","1991","1992","1993","1994","1995","1996","1997","1998","1999","2000","2001","2002","2003","2004","2005","2006","2007") ]
...
```{r}
goalD[Income_group=="Low income"&Series_Name.x=="GDP (current US$)"]
```


```{r}
a<-1595555476 +578595521+246804571+230317883+585427546+6173712809+59160576+87702857+368098032+1341590682+406084197 +486617279+742779660+246457839+465381104
a/15
```

```{r}
mean_income_goalD[Income_group=="Low income"&Series_Name.x=="GDP (current US$)"]
```
...
Change data type
```{r}
PlotDT_Region$Date = PlotDT_region_mean$Date%>%  as.Date(format = "%Y")
```
...
```{r}
ggplot() +
      geom_line(data=PlotDT[`Country_Name`=="Belgium" &`Series_Name.x`=="GDP (current US$)"], aes(x=Date, y=Value,colour=" Selected Country"))+ geom_line(data=PlotDT_Region[`Series_Name.x`=="GDP (current US$)"], aes(x= Date, y = Value,colour = `Region`))
...
```{r}
ggplot() +
      geom_line(data=PlotDT[`Country_Name`=="France" &`Series_Name.x`=="GDP (current US$)"], aes(x=Date, y=Value,colour=" Selected Country"))+ geom_line(data=PlotDT_Income_group[`Series_Name.x`=="GDP (current US$)"], aes(x= Date, y = Value,colour = `Income_group`))
